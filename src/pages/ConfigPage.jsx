...
export default function ConfigPage() {
  ...
  if (!config) return <div className="p-6 text-center text-white">Loading config...</div>;

  return (
    <section className="bg-white rounded-md shadow-md p-6">
      <h1 className="text-3xl text-pink-400 mb-4"> ✦ Drone Config ✦</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="p-4 bg-pink-100 rounded">
          <div className="text-sm text-gray-500">Drone ID</div>
          <div className="text-xl font-medium text-gray-700">{config.drone_id}</div>
        </div>
        <div className="p-4 bg-pink-100 rounded">
          <div className="text-sm text-gray-500">Drone Name</div>
          <div className="text-xl font-medium text-gray-700">{config.drone_name}</div>
        </div>
        <div className="p-4 bg-pink-100 rounded">
          <div className="text-sm text-gray-500">Light</div>
          <div className="text-xl font-medium text-gray-700">{config.light}</div>
        </div>
        <div className="p-4 bg-pink-100 rounded">
          <div className="text-sm text-gray-500">Country</div>
          <div className="text-xl font-medium text-gray-700">{config.country}</div>
        </div>
      </div>
